...
# Hay Here
def hay_harvest():
    while num_items(Items.Hay) < item_caps["hay_cap"]:
        for i in range(ws):
            harvest_now()
...
# Wood Here		
def wood_harvest():
    while num_items(Items.Wood) < item_caps["wood_cap"]:
        for i in range(ws):
            harvest_now()
...
# Carrot Here	
def carrot_harvest():
    while num_items(Items.Carrot) < item_caps["carrot_cap"]:
        for i in range(ws):
            harvest_now()
            soil()
            water()
...
# Pumpkin Here	
def pumpkin_harvest():
    while num_items(Items.Pumpkin) < item_caps["pumpkin_cap"]:    
        clear()
        ready = []
        for _ in range(ws):
            col = []
        ...
        while True:
            ...
            if get_pos_x() != 0:
                move(East)
            if num_items(Items.Pumpkin) > item_caps["pumpkin_cap"] or num_items(Items.Carrot) < 50000:
                break
    ...
    while True:
        for i in range(map):
            ...
            if get_pos_x() == 0 and dir == West:
                dir = North
        if num_items(Items.Bone) > item_caps["bone_cap"]:
            break
...
def cactus():
    while True:
        ...
        move_to(0, 0)
        harvest()
        if num_items(Items.Cactus) > item_caps["cactus_cap"]:
            break

# Sunflower Here
def sunflower():
    while num_items(Items.Fertilizer) > item_caps["fert_min"]:
        cur = {0:0, 1:0}
        def move_to(x, y):
            global cur
        ...
        while True:
            ...
            if y == wSize - 1:
                move(East)
            move(North)
            if num_items(Items.Power) > item_caps["power_cap"]:
                break

# Poly Here
def poly():
    while num_items(Items.Carrot) < item_caps["carrot_cap"] or num_items(Items.Wood) < item_caps["wood_cap"]:
        chain = []
        clear()
        soil()
        water()
...
def maze_starter():
    while True:
        if create_maze():
            if treasure_hunt():
                continue
        if num_items(Items.Gold) > item_caps["gold_cap"]:
            break
...
def treasure_hunt():
    ...
    x = get_pos_x()
    y = get_pos_y()
    while num_items(Items.Gold) < item_caps["gold_cap"]:
        move(dir)
