...
# Hay Here
def hay_harvest():
    while num_items(Items.Hay) < item_caps[Items.Hay]:
        clear()        
        for i in range(ws):
            harvest_now()
            move(North)
...
# Wood Here		
def wood_harvest():
    while num_items(Items.Wood) < item_caps[Items.Wood]:
        clear()
        for i in range(ws):
            harvest_now()
...
# Carrot Here	
def carrot_harvest():
    while num_items(Items.Carrot) < item_caps[Items.Carrot]:
        clear()
        for i in range(ws):
            soil_prep()
...
# Pumpkin Here	
def pumpkin_harvest():
    while num_items(Items.Pumpkin) < item_caps[Items.Pumpkin]:
        clear()
        ready = []
        for _ in range(ws):
            col = []
...
def dino():
    while num_items(Items.Bone) < item_caps[Items.Bone]:
        clear()
        #set_world_size(8)
...
# Cactus Here
def cactus():
    while num_items(Items.Cactus) < item_caps[Items.Cactus]:
        clear()
...
# Sunflower Here
def sunflower():
    while num_items(Items.Power) < item_caps[Items.Power]:
        clear()    
        cur = {0:0, 1:0}
        def move_to(x, y):
            global cur
...
# Poly Here
def poly():
    while num_items(Items.Wood) < item_caps[Items.Wood] or num_items(Items.Carrot) < item_caps[Items.Carrot]:    
        clear()
        soil_prep()
        plant(Entities.Carrot)
...
# Maze Starter Here
def maze_starter(): 
    while num_items(Items.Gold) < item_caps[Items.Gold] and num_items(Items.Weird_Substance) >= item_caps[Items.Weird_Substance]/ 0.5:
        clear()
        if create_maze():
            if treasure_hunt():
                continue
